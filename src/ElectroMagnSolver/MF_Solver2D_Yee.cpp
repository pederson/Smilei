...
void MF_Solver2D_Yee::operator()( ElectroMagn *fields )
{
    ...
    Field2D *Jmx2D = static_cast<Field2D *>( fields->Jmx_ );
    Field2D *Jmy2D = static_cast<Field2D *>( fields->Jmy_ );
    Field2D *Jmz2D = static_cast<Field2D *>( fields->Jmz_ );
    

    if (Jmx2D && Jmy2D && Jmz2D){
        {
            #pragma omp simd
            for( unsigned int j=1 ; j<ny_d-1 ; j++ ) {
                ( *Bx2D )( 0, j ) += -dt*( *Jmx2D )( 0, j ) - dt_ov_dy * ( ( *Ez2D )( 0, j ) - ( *Ez2D )( 0, j-1 ) );
            }
        }
        ...
        for( unsigned int i=1 ; i<nx_d-1;  i++ ) {
            #pragma omp simd
            for( unsigned int j=1 ; j<ny_d-1 ; j++ ) {
                ( *Bx2D )( i, j ) += -dt*( *Jmx2D )( i, j ) - dt_ov_dy * ( ( *Ez2D )( i, j ) - ( *Ez2D )( i, j-1 ) );
            }
            //    }
            ...
            for( unsigned int j=0 ; j<ny_p ; j++ ) {
                ( *By2D )( i, j ) += -dt*( *Jmy2D )( i, j ) + dt_ov_dx * ( ( *Ez2D )( i, j ) - ( *Ez2D )( i-1, j ) );
            }
            //}
            ...
            #pragma omp simd
            for( unsigned int j=1 ; j<ny_d-1 ; j++ ) {
                ( *Bz2D )( i, j ) += -dt*( *Jmz2D )( i, j ) + dt_ov_dy * ( ( *Ex2D )( i, j ) - ( *Ex2D )( i, j-1 ) )
